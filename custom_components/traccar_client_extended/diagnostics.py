...
from .attributes import describe, describe_fields, is_known
...
def _classify(description: Any) -> dict[str, Any]:
    """How this integration would publish one description, or that it would not."""
    if description is None:
        return {"entity": None, "device_class": None, "unit": None}
    return {
        "entity": description.key,
        "device_class": str(getattr(description, "device_class", None)),
    ...
    for device_id, record in (coordinator.data or {}).items():
        ...
        for key, value in record["attributes"].items():
            ...
            classified[key] = {
                ...
                **_classify(describe(key, value, override)),
                # A packed parameter produces a sensor per field as well as, or
                # instead of, one for the word itself -- and those are the
                # entities a report is most likely to be about, since their
                # unit and device class come from the profile rather than from
                # anything visible in the raw attribute.
                "fields": {
                    field.key: _classify(field)
