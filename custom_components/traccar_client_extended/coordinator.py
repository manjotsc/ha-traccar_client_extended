...
class TraccarClientExtendedCoordinator(DataUpdateCoordinator[TraccarCoordinatorData]):
    ...
    #: Cached uniqueId -> model subentry lookup, and the key it was built for.
    #: Class-level so the pure-logic tests, which build coordinators with
    #: `object.__new__`, do not each have to know about it.
    _subentry_index: dict[str, ConfigSubentry] | None = None
    _subentry_index_for: tuple[int, ...] | None = None
    ...
    def __init__(
        ...
        # Every tracker hangs off one hub device, which is the Traccar server
        # itself -- the one thing they all genuinely do connect through. Keyed
        # on the entry rather than the host so two entries pointing at the same
        # server stay separate, and so renaming the host does not orphan it.
        self.hub_identifier: tuple[str, str] = (
            DOMAIN,
            f"server_{config_entry.entry_id}",
        )
    ...
    async def _async_update_data(self) -> TraccarCoordinatorData:
        ...
        # Kept because it is the only complete list: `data` below is built from
        # positions, so a device with no stored fix never appears in it.
        ...
        # Every device gets a record, whether or not Traccar has a fix for it.
        # `/api/positions` returns the latest position per device by joining
        # `tc_devices.positionid`, so a device that has never reported -- or
        # whose positions the server has since cleaned up -- has no row there
        # at all. Keying the fleet off positions therefore made a long-quiet
        # device invisible: no record, so no entities, so no device in Home
        # Assistant, however plainly Traccar listed it.
        #
        # What it still knows is on the device record -- its name, its status,
        # when it was last heard from -- and that is worth publishing. The
        # position-derived entities read an empty dict and report nothing,
        # which is the honest answer to "where is it".
        for device in devices:
            if (device_id := device.get("id")) is None or device_id in data:
                continue
            last = previous.get(device_id)
            data[device_id] = self._build(
                device, last["position"] if last else PositionModel()
            )
    ...
    def _subentry_map(self) -> dict[str, ConfigSubentry]:
        """Return the live Traccar uniqueId -> model subentry lookup.

        Many-to-one: one subentry covers a model and holds every device of it,
        so building this is O(subentries x devices) and the accuracy filter
        alone asks for it twice per position. Rebuilding it every time made the
        cost quadratic in the fleet, the same way a linear `record_for` did.

        Cached on a tuple of the subentry object ids -- O(subentries), against
        O(subentries x devices) to rebuild. That key catches a subentry being
        added, removed or replaced. It deliberately does **not** catch one
        being edited in place, because it does not need to: Home Assistant's
        `async_update_subentry` mutates the ConfigSubentry with
        `object.__setattr__` rather than swapping it, so the object already in
        this map carries the new data.

        What the key cannot see is the *devices map* changing inside a
        subentry, which alters the keys rather than the values. Only this class
        does that, in `_add_to_model_subentry`, and `async_sync_shape` does it
        at setup -- both invalidate explicitly.
        """
        subentries = self.config_entry.subentries
        key = tuple(map(id, subentries.values()))
        if self._subentry_index_for == key and self._subentry_index is not None:
            return self._subentry_index

        found: dict[str, ConfigSubentry] = {}
        for subentry in subentries.values():
            if subentry.subentry_type != SUBENTRY_TYPE_DEVICE:
                continue
            for unique_id in device_entries(subentry):
                found[unique_id] = subentry

        self._subentry_index = found
        self._subentry_index_for = key
        return found

    @callback
    def invalidate_subentry_index(self) -> None:
        """Drop the cached lookup after the devices map inside one has changed.

        Editing a subentry's data does not need this -- Home Assistant mutates
        the object in place, so the map already points at the new data. Adding
        or removing a *device* does, because that changes the map's keys.
        """
        self._subentry_index = None
        self._subentry_index_for = None

    def subentry_for(self, device_unique_id: str | None) -> ConfigSubentry | None:
        """Return the model subentry a device belongs to, if it has one."""
        if device_unique_id is None:
            return None
        return self._subentry_map().get(device_unique_id)

    def device_settings_for(self, device_unique_id: str | None) -> dict[str, Any]:
        """Return one device's own settings from inside its model subentry."""
        if (subentry := self.subentry_for(device_unique_id)) is None:
            return {}
        return device_entries(subentry).get(device_unique_id, {})

    def _layers_for(
        self, device_unique_id: str | None
    ) -> tuple[ConfigSubentry | None, dict[str, Any]]:
        """Return the model subentry and the device's own settings, in one go."""
        if (subentry := self.subentry_for(device_unique_id)) is None:
            return None, {}
        return subentry, device_entries(subentry).get(device_unique_id, {})
    ...
    def overrides_for(
        ...
        Entry-level overrides apply to every device, a model subentry's to
        every device of that model, and one device's own on top of both so a
        single tracker can reinterpret a key differently.
        """
        ...
        subentry, device = self._layers_for(device_unique_id)
        if subentry is not None:
            merged.update(parse_overrides(subentry.data.get(CONF_ATTRIBUTE_OVERRIDES)))
        # And one tracker's own overrides on top of its model's, for the van
        # with the aftermarket sensor nobody else has.
        merged.update(parse_overrides(device.get(CONF_ATTRIBUTE_OVERRIDES)))
        return merged
    ...
    def device_models(self) -> dict[str, str | None]:
        """Return ``{uniqueId: model}`` for every device on the server.

        From the device list rather than `self.data`, which is keyed off
        *positions*: a device Traccar has never stored a fix for is absent from
        that, and reading it here filed every such device under "Unspecified
        model" no matter what its model said.

        Blanks are kept rather than dropped: a device with no model still needs
        a subentry, and they collect in one of their own.
        """
        return {
            unique_id: device.get("model")
            for device in self._devices
            if (unique_id := device.get("uniqueId"))
        }
    ...
    def profile_for(self, device_unique_id: str | None) -> DeviceProfile | None:
        ...
        An explicit choice on the model's subentry always wins, including an
        explicit "none". Otherwise the model is matched against the profile
        library. The choice is per model rather than per device because a
        profile describes hardware, and the subentry now *is* the model.
        """
        choice = PROFILE_AUTO
    ...
    def max_accuracy_for(self, device_unique_id: str | None) -> float:
        ...
        Most specific wins: the device, then its model, then the server-wide
        value. A phone needs a filter where a hardwired tracker does not, and
        that is the one setting where the exception is per device rather than
        per model.
        """
        subentry, device = self._layers_for(device_unique_id)
        value = device.get(CONF_MAX_ACCURACY)
        if value is None and subentry is not None:
            value = subentry.data.get(CONF_MAX_ACCURACY)
        return float(value) if value is not None else self.max_accuracy
        ...
        if self.auto_add_devices:
            ...
            for device in devices:
                ...
                if not unique_id or unique_id in existing or unique_id in known:
                    continue
                self._add_to_model_subentry(unique_id, device)
    ...
    def _add_to_model_subentry(self, unique_id: str, device: DeviceModel) -> None:
        """Put a device in its model's subentry, creating the subentry if new.

        Creating one is the common path on a first run and the rare path after,
        since a fleet usually shares a handful of models.
        """
        model = device.get("model")
        settings = device_settings(device.get("name") or unique_id, device["id"])

        if (subentry := find_subentry(self.config_entry, model)) is None:
            self.hass.config_entries.async_add_subentry(
                self.config_entry,
                new_subentry(model, self.profiles, {unique_id: settings}),
            )
            return
        ...
        self.hass.config_entries.async_update_subentry(
            self.config_entry,
            subentry,
            data=with_device(subentry, unique_id, settings),
        )
        self.invalidate_subentry_index()
    ...
    async def _api(
        ...
        The body is read rather than tested for a Content-Length, because that
        header is absent from every chunked response -- which is what Traccar
        answers with the moment it sits behind a reverse proxy that gzips. A
        `not response.content_length` guard therefore threw the notification
        list away on exactly the deployments most people run, and an empty list
        reads here as "no web notification exists": the repair fires on a
        correctly configured server, and with `create_notifications` on, all 23
        notifications are created again on every single setup.
        """
        async with self._session.request(
            ...
            if not raw.strip():
                return None
            # Parsed rather than `response.json()`, which insists on a JSON
            # content type: Traccar answers `POST /api/notifications` as
            # application/json, but a proxy rewriting it must not cost us the
            # body we can plainly see.
            try:
                return json.loads(raw)
            except ValueError:
                LOGGER.debug("Traccar returned a non-JSON body for %s", path)
                return None
