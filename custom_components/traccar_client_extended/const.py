...
# One subentry per device *model*, holding every device of it. The type string
# still says "device" because changing it would orphan every subentry already
# written. A mixed fleet needs the grouping: `io800` means different things on
# different Teltonika models, so the overrides and the profile belong to the
# model, while a name and an accuracy filter stay per device inside it.
SUBENTRY_TYPE_DEVICE: Final = "device"
#: The model a sub-entry covers, as Traccar spells it. Empty for the sub-entry
#: that collects devices whose model field nobody filled in.
CONF_MODEL: Final = "model"
#: Per-device settings inside a model sub-entry, keyed by Traccar uniqueId.
CONF_DEVICES: Final = "devices"
#: Which device the per-device step is editing. Not stored.
CONF_EDIT_DEVICE: Final = "edit_device"
...
#: A one-shot action rather than a setting: ticking it enables this device's
#: disabled entities on save, and it is never written to the subentry. Storing
#: it would re-enable, on every later save, whatever the user had deliberately
#: switched off in Home Assistant's own entity settings.
CONF_ENABLE_ALL_ENTITIES: Final = "enable_all_entities"
#: The undo for the above, and a one-shot action for the same reason: put every
#: entity back to the enabled state its mapping asks for. Not "disable all",
#: which would take out the device tracker and every real reading too.
CONF_RESET_ENTITIES: Final = "reset_entities"
...
ATTR_TRACCAR_OPERATOR: Final = "traccar_operator"
ATTR_RAW_VALUE: Final = "raw_value"
