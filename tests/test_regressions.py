...
def test_every_traccar_event_has_a_readable_label() -> None:
    ...
    from `const.EVENT_LABELS` rather than a translation key -- hassfest requires
    a key to match [a-z0-9-_]+, which Traccar's camelCase does not. A type added
    to EVENTS without a label renders as the raw API string -- `proximityExit`
    rather than "Linked device away" -- which matches nothing the user has seen
    in Traccar's own UI.
    """
    ...
    assert all(label and label != event for event, label in labels.items())
...
def _subscribe_source() -> str:
    """The subscribe loop, read as text: both guards below are about what it
    must and must not contain, and standing up a live websocket to assert that
    would test aiohttp rather than this."""
    from custom_components.traccar_client_extended.coordinator import (
        TraccarClientExtendedCoordinator,
    )

    return inspect.getsource(TraccarClientExtendedCoordinator.subscribe)
...
def test_reconnect_resyncs_device_state() -> None:
    ...
    sensor keeps the value it had before the gap.
    """
    assert "async_request_refresh" in _subscribe_source()
...
def test_diagnostics_classify_through_profiles_and_overrides() -> None:
    ...
    assert "override = overrides.get(key)" in text
    assert "describe(key, value, override)" in text
