...
def subentry(unique_id: str, choice: str, model: str = "TEST") -> ConfigSubentry:
    """A model subentry holding one device.

    The profile choice sits on the subentry rather than the device: a profile
    describes hardware, and the subentry is the model.
    """
    return ConfigSubentry(
        data=MappingProxyType(
            {
                CONF_MODEL: model,
                CONF_DEVICES: {unique_id: {}},
                CONF_DEVICE_PROFILE: choice,
            }
        ),
