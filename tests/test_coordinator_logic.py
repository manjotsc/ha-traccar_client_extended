...
def make_coordinator(**overrides):
    ...
    # The subentry lookup reads this; an entry with no sub-entries is the
    # honest default for a coordinator built field by field.
    coordinator.config_entry = SimpleNamespace(data={}, subentries={})
...
# -- Devices with no stored fix ----------------------------------------------


def refresh(devices, positions):
    """Run the record-building half of `_async_update_data`."""
    coordinator = make_coordinator()
    ...
    coordinator.config_entry = SimpleNamespace(data={}, subentries={})
    coordinator.auto_add_devices = False

    data = {}
    for position in positions:
        device = next(d for d in devices if d["id"] == position["deviceId"])
        data[device["id"]] = coordinator._build(device, position)
    for device in devices:
        if device["id"] not in data:
            data[device["id"]] = coordinator._build(device, {})
    return data


def test_a_device_with_no_position_still_gets_a_record() -> None:
    """`/api/positions` lists the latest fix per device, so a tracker that has
    never reported is absent from it -- and used to vanish entirely."""
    devices = [
        {"id": 1, "uniqueId": "quiet", "name": "Quiet", "status": "offline"},
        {"id": 2, "uniqueId": "live", "name": "Live", "status": "online"},
    ]
    positions = [{"deviceId": 2, "id": 9, "latitude": 1.0, "longitude": 2.0}]

    data = refresh(devices, positions)

    assert set(data) == {1, 2}
    assert data[1]["position"] == {}
    assert data[1]["attributes"] == {}
    assert data[1]["device"]["status"] == "offline"


def test_a_device_with_no_position_has_no_geofences() -> None:
    data = refresh([{"id": 1, "uniqueId": "quiet"}], [])
    assert data[1]["geofences"] == []
