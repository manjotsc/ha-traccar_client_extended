...
class _Entries:
    ...
    def async_update_subentry(self, entry, subentry, *, data) -> None:
        replaced = ConfigSubentry(
            data=MappingProxyType(data),
            subentry_type=subentry.subentry_type,
            title=subentry.title,
            unique_id=subentry.unique_id,
        )
        entry.subentries[subentry.subentry_id] = replaced
        self.added.append(replaced)
...
def subentry(unique_id: str, model: str = "TEST") -> ConfigSubentry:
    return ConfigSubentry(
        data=MappingProxyType({CONF_MODEL: model, CONF_DEVICES: {unique_id: {}}}),
...
def added(c) -> list[str]:
    """Every device that ended up in a subentry, in the order it was added."""
    seen: list[str] = []
    for subentry in c.hass.config_entries.added:
        seen.extend(
            unique_id
            for unique_id in subentry.data.get(CONF_DEVICES, {})
