...
def test_no_option_is_owned_by_two_steps() -> None:
    ...
    for step_id, step in OPTIONS_FLOW.items():
        if step_id == "init":
            continue
        # Resolved rather than read: the attributes step builds its picker from
        # live data, so its schema is a coroutine function.
...
async def test_every_setting_on_the_step_is_saved() -> None:
    """What this returns *is* the step's saved options.

    A setting left out of the returned dict is discarded with no error, and the
    form redisplays the old value as though nothing was submitted. That shipped
    once: `group_by_model` was added to the schema, never added here, and the
    toggle did nothing at all.
    """
    h = handler()
    schema_keys = {str(key) for key in OPTIONS_FLOW["devices"].schema.schema}

    saved = await _save_devices(h, {CONF_AUTO_ADD_DEVICES: True})

    # CONF_CONFIRM is the one deliberate exception: it is an action.
    assert set(saved) == schema_keys - {CONF_CONFIRM}
